...
namespace NeuralNetwork1
{
    ...
    public class Sample
    {
        ...
        /// </summary>
        public NumType actualClass;

        /// <summary>
        /// Распознанный класс - определяется после обработки
        /// </summary>
        public NumType recognizedClass;
        ...
        public Sample(double[] inputValues, int classesCount, NumType sampleClass = NumType.Undef)
        {
            //  Клонируем массивчик
            input = (double[]) inputValues.Clone();
            Output = new double[classesCount];
            if (sampleClass != NumType.Undef) Output[(int) sampleClass] = 1;


            recognizedClass = NumType.Undef;
        ...
        /// </summary>
        public NumType ProcessPrediction(double[] neuralOutput)
        {
            Output = neuralOutput;
            if (error == null)
            ...
            for (int i = 0; i < Output.Length; ++i)
            {
                ...
                if (Output[i] > Output[(int) recognizedClass]) recognizedClass = (NumType) i;
            }
